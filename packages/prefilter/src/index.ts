...
// Each dst texel averages exactly its 2x2 src texels, weighted by their
// solid angles — a gapless partition of the source. The previous "angular"
// downsampler took 9 sparse taps up to 4 src texels apart, leaving gaps
// between tap footprints; spiky content (analytical lights spanning a few
// texels) aliased into a lumpy pyramid, which the specular pass's filtered
// importance sampling then turned into structured noise.
const BOX_DOWNSAMPLE_WGSL = `
@group(0) @binding(0) var src: texture_2d_array<f32>;
@group(0) @binding(1) var dst: texture_storage_2d_array<rgba32float, write>;

// Solid angle of a cubemap texel centered at face coords (u, v) in [-1, 1]
// is proportional to (1 + u^2 + v^2)^-1.5.
fn texel_weight(coord: vec2<i32>, size: u32) -> f32 {
  let uv = (vec2<f32>(coord) + 0.5) / f32(size) * 2.0 - 1.0;
  let d = 1.0 + dot(uv, uv);
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
  let dst_size = textureDimensions(dst).x;
  if (gid.x >= dst_size || gid.y >= dst_size || gid.z >= 6u) { return; }
  ...
  for (var dy = 0; dy < 2; dy++) {
    for (var dx = 0; dx < 2; dx++) {
      let coord = base + vec2(dx, dy);
      let w = texel_weight(coord, src_size);
      color += textureLoad(src, coord, layer, 0) * w;
      total += w;
    }
  }
  textureStore(dst, gid.xy, layer, color / total);
}
`;
...
function generateCubemapMips(
  ...
  const module = device.createShaderModule({ code: BOX_DOWNSAMPLE_WGSL });
  ...
  // One pass per mip: a mip written by one pass may only be read in a later
  // pass (same-pass read+write of one subresource fails usage validation).
